...
RESULTS_DIR    = os.path.join(BASE_DIR, "results_nb")
CHECKPOINT_DIR = os.path.join(BASE_DIR, "checkpoints_nb")
CACHE_DIR      = os.path.join(BASE_DIR, "cache_nb")

VAR_SMOOTHING = 1.32e-08
...
def run_pipeline():
    ...
    json_path = os.path.join(RESULTS_DIR, "nb_results.json")
            ...
            for outer_idx, fold_data in enumerate(nested_folds):
                ...
                if outer_idx == n_outer - 1:
                    cm_path = os.path.join(
                        RESULTS_DIR, f"cm_nb_seed{seed}_size{data_size}.png")
                    plot_confusion_matrix(y_test, preds,
                        f"NB CM (Size={data_size}, Seed={seed})",
                        save_path=cm_path)
                    rel_path = os.path.join(
                        RESULTS_DIR, f"reliability_nb_seed{seed}_size{data_size}.png")
    ...
    print("Naive Bayes Pipeline completed.")
