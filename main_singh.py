"""
BiLSTM Classifier Progressive Training Pipeline
"""
import os
import time
import random
...
RESULTS_DIR    = os.path.join(BASE_DIR, "results_singh")
CHECKPOINT_DIR = os.path.join(BASE_DIR, "checkpoints_singh")
CACHE_DIR      = os.path.join(BASE_DIR, "cache_singh")
...
def run_pipeline():
    ...
    print(f"BiLSTM Pipeline (Singh et al. 2023)")
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for stage_idx, data_size in enumerate(data_sizes):
            ...
            print(f"\n{'='*60}")
            print(f"Stages {stage_idx+1}/{len(data_sizes)}: Size={data_size}, Seed={seed}")
            print(f"{'='*60}")
