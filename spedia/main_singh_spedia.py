"""
BiLSTM (Singh et al. 2023) — SPEDIA Dataset
"""
import os, sys
...
def run_pipeline():
    ...
    device = cfg.DEVICE
    os.makedirs(cfg.RESULTS_SINGH_DIR, exist_ok=True)
    ...
    print(f"BiLSTM — SPEDIA")
    print(f"Architecture: PCA(16) → 3-layer BiLSTM(256) → Linear(512→1)")
    print(f"Epochs={cfg.BILSTM_EPOCHS}  LR={cfg.BILSTM_LR}  BatchSize={cfg.BILSTM_BATCH}")
    print(f"Device: {device}  |  Seeds: {seeds}  |  Data Sizes: {data_sizes}")
    print(f"{'='*60}\n")

    json_path = os.path.join(cfg.RESULTS_SINGH_DIR, "singh_results.json")
        ...
        for stage_idx, data_size in enumerate(data_sizes):
            ...
            for outer_idx, fold_data in enumerate(nested_folds):
                ...
                model = BiLSTMClassifier(
                    hidden_size=cfg.BILSTM_HIDDEN, num_layers=cfg.BILSTM_LAYERS
                ).to(device)

                train_latency = train_model(
                    model, X_train, y_train,
                    cfg.BILSTM_EPOCHS, cfg.BILSTM_BATCH, cfg.BILSTM_LR, device
                )
                ...
                if fold_metrics["f1"] > best_outer_score:
                    ...
                    torch.save({"model_state": model.state_dict(), "epoch": cfg.BILSTM_EPOCHS},
                               os.path.join(cfg.CHECKPOINT_SINGH_DIR,
                                            f"singh_seed{seed}_size{data_size}_best.pt"))

                print(f"Fold {outer_idx+1} (train {train_latency:.1f}s): "
                ...
                if outer_idx == n_outer - 1:
                    plot_confusion_matrix(y_test, preds,
                        ...
                        save_path=os.path.join(cfg.RESULTS_SINGH_DIR,
                            f"cm_singh_seed{seed}_size{data_size}.png"))
                    plot_reliability_diagram(y_test, probs,
                        f"BiLSTM Calibration (Size={data_size}, Seed={seed})",
                        save_path=os.path.join(cfg.RESULTS_SINGH_DIR,
                            f"reliability_singh_seed{seed}_size{data_size}.png"))
