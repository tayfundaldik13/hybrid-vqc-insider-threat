...
def train_and_evaluate(X_train, y_train, X_test, y_test, seed):
    model = SVC(C=cfg.SVM_C, kernel=cfg.SVM_KERNEL, gamma=cfg.SVM_GAMMA,
...
def run_pipeline():
    ...
    os.makedirs(cfg.RESULTS_SVM_DIR, exist_ok=True)
    ...
    print(f"SVM — SPEDIA")
    print(f"SVC C={cfg.SVM_C} kernel={cfg.SVM_KERNEL} gamma={cfg.SVM_GAMMA}")
    print(f"Seeds: {seeds}  |  Data Sizes: {data_sizes}")
    print(f"{'='*60}\n")

    json_path = os.path.join(cfg.RESULTS_SVM_DIR, "svm_results.json")
            ...
            for outer_idx, fold_data in enumerate(nested_folds):
                ...
                if outer_idx == n_outer - 1:
                    plot_confusion_matrix(y_test, preds,
                        ...
                        save_path=os.path.join(cfg.RESULTS_SVM_DIR,
                            f"cm_svm_seed{seed}_size{data_size}.png"))
                    plot_reliability_diagram(y_test, probs,
                        f"SVM Calibration (Size={data_size}, Seed={seed})",
                        save_path=os.path.join(cfg.RESULTS_SVM_DIR,
                            f"reliability_svm_seed{seed}_size{data_size}.png"))
